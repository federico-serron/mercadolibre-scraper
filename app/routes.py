...
def index():
    ...
    if request.method == 'GET':
        ...
        if form.validate_on_submit():
            ...
            links = mlscraping.get_list(form.country.data, term_to_search)
        
            if len(links) is not None:
                products = mlscraping.makeScrap(links)
                avg_sales = [int(k['sells']) for k in products if k['sells'] is not None]
                if len(avg_sales) < 1:
                    avg_sales = 0
                else:
                    avg_sales = round(np.mean(avg_sales))
